...
## localhost:5000/
@app.route("/") 
# @app.route() --> localhost: 5000/ 요청이 있는 경우에 바로 아래 있는 함수
# 함수를 실행 
def index():
    return render_template("index.html")


#get 형시에서 데이터는 url에 실어서 보낸다.
#request -> 유저가 서버에게 보내는 데이터를 dict형태로 출력
...
# id값과 password 값
# id는 test, password 는 1234
# 위의 조건이 만조해야만 second.html 리턴
...
# Flask라는  Class안에 있는 run()이라는 함수 호출
app.run()
