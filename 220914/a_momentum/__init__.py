...
class A_moment():
    ...
    def create_df(self):
        for i in self.month_list:
            ...
            #month_last_df의 길이와 같은 길이 값을 가진 변수는?
            #len(month_last_df) ==len(?) -> True
    ...
    def add_trade(self):
        for i in self.month_last_df.index:
            ...
            if self.flag:
                self.signal = "buy"
            print("날짜 : ", i, "모멘텀 인덱스 : ", self.momentum_index, "flag : ", self.flag, "signal : ", self.signal)
    ...
    def returns(self):
        ...
        print("누적수익율 : ", round(self.acc_rtn,4))
        return self.df
